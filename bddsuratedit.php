...
<div class="main">
  ...
  <div class="panel-body">
         <?php
    $id = $_GET['id_surat'];
    $result = mysqli_query($koneksi,"SELECT * FROM surat WHERE id_surat='$id'") or die(mysqli_error());
    $no=1; 
    while ($data = mysqli_fetch_array($result)) { //fetch the result from query into an array
    ?>
    <form action="bddsuratupdate.php" method="POST"> 
       <div class="form-group">
       <label for="id_surat">ID Surat: </label>
       <input type="text" class="form-control" id="id_surat" name="id_surat" readonly="" value="<?php echo $data['id_surat'] ?>">  
        </div>        
    ...
    <option>- SPK -</option>
    <?php
      $aspk="SELECT * FROM spk";     
      $sqlspk=mysqli_query($koneksi, $aspk);
      //$id_surat = $datasurat['id_surat'];
     while($dataspk=mysqli_fetch_array($sqlspk)){
  ?>
        <option value="<?php echo $dataspk['id_spk']; ?>" <?php if ($dataspk['id_spk'] == $data['id_spk']) { echo 'selected'; } ?>><?php echo "ID SPK : "; echo $dataspk['id_spk']; echo " -- No. SPK : "; echo $dataspk['no_spk']?></option>
<?php } ?>
       </select> 
        </div>   
    ...
    <option>- SPKI -</option>
    <?php
      $aspki="SELECT * FROM spki";
      $sqlspki=mysqli_query($koneksi, $aspki);
     while($dataspki=mysqli_fetch_array($sqlspki)){
  ?>
        <option value="<?php echo $dataspki['id_spki']; ?>" <?php if ($dataspki['id_spki'] == $data['id_spki']) { echo 'selected'; } ?>><?php echo "ID SPKI : "; echo $dataspki['id_spki']; echo " -- No. SPKI : "; echo $dataspki['no_spki']?>
      </option>
<?php } ?>
       </select> 
        </div>        
         ...
         <div class="form-group">
       <label for="id_objek">Surat Tugas: </label>
      <select class="form-control" name="id_objek" id="id_objek" required="">
    <option>- SURAT TUGAS -</option>
     <?php
      $asrtt="SELECT * FROM surat_tugas";
      //$id_objek = $dataobj['id_objek'];
      $sqlsrtt=mysqli_query($koneksi, $asrtt);
     while($datasrtt=mysqli_fetch_array($sqlsrtt)){
  ?>
        <option value="<?php echo $datasrtt['id_surat_tugas']; ?>" <?php if ($datasrtt['id_surat_tugas'] == $data['id_surat_tugas']) { echo 'selected'; } ?>><?php echo "ID Surat Tugas : "; echo $datasrtt['id_surat_tugas']; echo " -- No. Surat : "; echo $datasrtt['no_surat']?>
      </option>
<?php } ?>
       </select> 
        </div>      
       ...
       <select class="form-control" id="id_opp" name="id_opp" required="required">
                <option>- OPP -</option>
                 <?php
      $aopp="SELECT * FROM opp";
      $sqlopp=mysqli_query($koneksi, $aopp);
      //$id_fee = $datafee['id_fee'];
     while($dataopp=mysqli_fetch_array($sqlopp)){
  ?>
        <option value="<?php echo $dataopp['id_opp']; ?>" <?php if ($dataopp['id_opp'] == $data['id_opp']) { echo 'selected'; } ?>><?php echo "ID OPP : "; echo $dataopp['id_opp']; echo " -- No. OPP : "; echo $dataopp['no_opp']?></option>
<?php } ?>
       </select> 
        </div>          
