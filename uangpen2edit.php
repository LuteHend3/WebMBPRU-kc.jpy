...
<div class="main">
  ...
  <div class="panel-body">
    ...
    $id = $_GET['id_pen2'];
    $result = mysqli_query($koneksi,"SELECT * FROM penagihan2 WHERE id_pen2='$id'") or die(mysqli_error());
    $no=1; 
    while ($data = mysqli_fetch_array($result)) { //fetch the result from query into an array
    ?>
    <form action="uangpen2update.php" method="POST"> 
        <div class="form-group">
       <label for="id_pen2">ID Penagihan 2 : </label>
       <input type="text" class="form-control" id="id_pen2" name="id_pen2" readonly="" value="<?php echo $data['id_pen2'] ?>">  
        </div>        

 <div class="form-group">
       <label for="no_invoice">Nomor Invoice : </label>
       <input type="text" class="form-control" id="no_invoice" name="no_invoice" required="required" value="<?php echo $data['no_invoice'] ?>">  
        </div>  

       
 <div class="form-group">
       <label for="tgl_pen2">Tanggal Penagihan 2 : </label>
       <input type="text" class="form-control" id="tgl_pen2" name="tgl_pen2" required="required" value="<?php echo $data['tgl_pen2'] ?>">  
        </div>   

      <div class="form-group">
       <label for="no_faktur">Nomor Faktur : </label>
       <input type="text" class="form-control" id="no_faktur" name="no_faktur" required="required" value="<?php echo $data['no_faktur'] ?>">  
        </div>  

         <div class="form-group">
       <label for="tgl_faktur">Tanggal Faktur : </label>
       <input type="text" class="form-control" id="tgl_faktur" name="tgl_faktur" required="required" value="<?php echo $data['tgl_faktur'] ?>">  
        </div>   
