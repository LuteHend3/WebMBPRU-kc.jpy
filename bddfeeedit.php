...
<div class="main">
  ...
  <div class="panel-body">
    ...
    $id = $_GET['id_fee'];
    $result = mysqli_query($koneksi,"SELECT * FROM fee WHERE id_fee='$id'") or die(mysqli_error());
    $no=1; 
    while ($data = mysqli_fetch_array($result)) { //fetch the result from query into an array
    ?>
    <form action="bddfeeupdate.php" method="POST"> 
        <div class="form-group">
       <label for="id_fee">ID Fee: </label>
       <input type="text" class="form-control" id="id_fee" name="id_fee" readonly="" required="required" value="<?php echo $data['id_fee'] ?>">  
        </div>        

        <div class="form-group">
       <label for="pro_fee">Pro Fee: </label>
       <input type="text" class="form-control" id="pro_fee" name="pro_fee" onkeyup="sum();" required="required" value="<?php echo $data['pro_fee'] ?>">  
        </div>   

       
        <div class="form-group">
       <label for="transport">Transport : </label>
       <input type="text" class="form-control" id="transport" name="transport" onkeyup="sum();" required="required" value="<?php echo $data['transport'] ?>">  
        </div>    

         <div class="form-group">
       <label for="fee_bank">Fee bank : </label>
       <input type="text" class="form-control" id="fee_bank" name="fee_bank" onkeyup="sum();" required="required" value="<?php echo $data['fee_bank'] ?>">  
        </div>  

         <div class="form-group">
       <label for="ppn">PPN : </label>
       <input type="text" class="form-control" id="ppn" name="ppn" onkeyup="sum();" required="required" value="<?php echo $data['ppn'] ?>">  
        </div>   

        <div class="form-group">
       <label for="fee_luar">Fee Luar : </label>
       <input type="text" class="form-control" id="fee_luar" name="fee_luar" onkeyup="sum();" required="required" value="<?php echo $data['fee_luar'] ?>">  
        </div>   

        <div class="form-group">
       <label for="total">Total : </label>
       <input type="text" class="form-control" id="total" name="total" required="required" value="<?php echo $data['total'] ?>">  
        </div>      
